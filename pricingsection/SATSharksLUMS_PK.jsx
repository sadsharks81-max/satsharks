...
const trackRecord = ["International Math Olympiad medalists","National Science competition winners","International debate and MUN champions","National level hackathon finalists","Published research authors","Social enterprise founders"];
...
export default function SATSharksLUMSPK() {
  ...
  const [form, setForm] = useState({ name:"", email:"", phone:"", city:"", program:"" });
  ...
  return (
    <div className="min-h-screen bg-gray-50" style={{ fontFamily: "'Inter','Segoe UI',system-ui,sans-serif" }}>
      ...
      {/* Hero */}
      <section className="px-5 pt-14 pb-10 text-center" style={{ background: "linear-gradient(165deg, #0f1b2d 0%, #1a2744 50%, #1e3054 100%)" }}>
        <span className="inline-block text-xs font-bold tracking-widest uppercase px-4 py-1.5 rounded-full mb-6" style={{ backgroundColor: "#f59e0b", color: "#0f1b2d" }}>LUMS Admissions Counselling</span>
        <h1 className="text-3xl font-extrabold text-white leading-tight mb-2">LUMS Doesn't Pick</h1>
        <h1 className="text-3xl font-extrabold leading-tight mb-5" style={{ color: "#f59e0b" }}>The Loudest Applicant.</h1>
        <p className="text-sm leading-relaxed max-w-md mx-auto mb-8" style={{ color: "#94a3b8" }}>They pick the one with the clearest story. We help you find yours and write it in a way the admissions committee won't forget.</p>
        <div className="flex justify-center gap-6 mb-6">
          {[{ num:"98%", label:"Acceptance 2026", color:"#22c55e" },{ num:"100%", label:"Acceptance 2025", color:"#f59e0b" },{ num:"All", label:"Programs Covered", color:"#60a5fa" }].map(s => (
            <div key={s.label} className="text-center"><div className="text-2xl font-extrabold" style={{ color: s.color }}>{s.num}</div><div className="text-xs mt-0.5" style={{ color: "#64748b" }}>{s.label}</div></div>
          ))}
        </div>
        <p className="text-xs mb-6" style={{ color: "#64748b" }}>All results verified on our Instagram</p>
      ...
      {/* Track Record */}
      <section className="px-5 py-6 text-center" style={{ backgroundColor: "#f8fafc", borderBottom: "1px solid #e5e7eb" }}>
        <div className="flex items-center justify-center gap-2 mb-4"><span className="text-base">🏆</span><h3 className="text-xs font-bold tracking-widest uppercase" style={{ color: "#0f1b2d" }}>Our Students' Track Record</h3></div>
        <div className="flex flex-wrap justify-center gap-2 max-w-xl mx-auto">
          {trackRecord.map(t => (<span key={t} className="text-xs px-3 py-1.5 rounded-full font-medium bg-white border border-gray-200" style={{ color: "#475569" }}>{t}</span>))}
        </div>
      </section>

      {/* What Sets Us Apart */}
      <section className="px-4 py-10 max-w-5xl mx-auto">
        <h2 className="text-center text-xl font-bold mb-2" style={{ color: "#0f1b2d" }}>We Don't Just Write Essays. We Build LUMS Applicants.</h2>
        <p className="text-center text-sm mb-8 max-w-lg mx-auto" style={{ color: "#64748b" }}>Most consultancies polish your existing profile. We build the profile LUMS actually wants to see.</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { icon:"🔬", title:"EC Developers", desc:"Dedicated team that designs and guides you through Math Olympiads, Science competitions, MUN, hackathons, and research projects. We create the achievements, not just describe them." },
            { icon:"✍️", title:"LUMS Specific Essays", desc:"Counsellors who know exactly what LUMS admissions looks for. Every word calibrated for LUMS, not generic university applications." },
            { icon:"📊", title:"98% Acceptance Rate", desc:"Our track record speaks for itself. 98% in 2026, 100% in 2025. Every claim verified on our Instagram." },
            { icon:"🎯", title:"Profile Gap Analysis", desc:"We analyze your profile against what LUMS values, identify exactly what is missing, and build those missing pieces before you apply." }
          ].map((d, i) => (
      ...
      <section className="px-4 py-6 max-w-5xl mx-auto">
        <h2 className="text-center text-xl font-bold mb-1" style={{ color: "#0f1b2d" }}>Choose Your Path to LUMS</h2>
        <p className="text-center text-sm mb-8" style={{ color: "#64748b" }}>Transparent pricing. No hidden fees. Results that speak.</p>
        <div className="flex flex-col lg:flex-row gap-5 items-stretch">
          ...
          <div className="flex-1 bg-white rounded-2xl border border-gray-200 overflow-hidden flex flex-col">
            <div className="px-6 pt-6 pb-4">
              ...
              <p className="text-xs mb-4" style={{ color: "#64748b" }}>We work alongside you to build an application that is authentically yours, with expert guidance shaping every section.</p>
              <div className="flex items-baseline gap-1 mb-1"><span className="text-3xl font-extrabold" style={{ color: "#0f1b2d" }}>Rs. 30,000</span></div>
              <p className="text-xs mb-4" style={{ color: "#94a3b8" }}>PKR</p>
            </div>
            <div className="px-6 pb-6 flex-1 flex flex-col">
              {guided.map((s, i) => (<div key={i} className={i > 0 ? "mt-4" : ""}><h4 className="text-sm font-bold mb-2" style={{ color: "#0f1b2d" }}>{s.cat}</h4><div className="flex flex-col gap-2">{s.items.map((it, j) => (<div key={j} className="flex gap-2.5 items-start"><CheckIcon /><span className="text-sm leading-snug" style={{ color: "#475569" }}>{it}</span></div>))}</div></div>))}
              <div className="mt-auto pt-6 flex flex-col gap-2.5">
                <button className="w-full py-3.5 rounded-xl text-white font-semibold text-sm" style={{ backgroundColor: "#0f1b2d" }} onClick={() => openReg("Guided Support")}>Register Now</button>
                <button className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl text-white font-semibold text-sm" style={{ backgroundColor: "#25D366" }} onClick={() => window.open(`https://wa.me/${WA}?text=${encodeURIComponent("Hi! I'm interested in the Guided Support package for LUMS.")}`, "_blank")}><WAIcon />Chat on WhatsApp</button>
              </div>
            </div>
          </div>
          {/* Complete */}
          <div className="flex-1 rounded-2xl overflow-hidden flex flex-col relative bg-white" style={{ border: "2px solid #f59e0b" }}>
            ...
            <div className="px-6 pt-6 pb-4">
              ...
              <p className="text-xs mb-4" style={{ color: "#64748b" }}>We handle everything, from writing your essays to putting you on the podium at competitions that LUMS actually cares about.</p>
              <div className="flex items-baseline gap-1 mb-1"><span className="text-3xl font-extrabold" style={{ color: "#0f1b2d" }}>Rs. 60,000</span></div>
              <p className="text-xs mb-4" style={{ color: "#94a3b8" }}>PKR</p>
            </div>
            <div className="px-6 pb-6 flex-1 flex flex-col">
              {complete.map((s, i) => (<div key={i} className={i > 0 ? "mt-4" : ""}><h4 className="text-sm font-bold mb-2" style={{ color: "#0f1b2d" }}>{s.cat}</h4><div className="flex flex-col gap-2">{s.items.map((it, j) => (<div key={j} className="flex gap-2.5 items-start"><StarIcon /><span className="text-sm leading-snug" style={{ color: "#475569" }}>{it}</span></div>))}</div></div>))}
              <div className="mt-auto pt-6 flex flex-col gap-2.5">
                <button className="w-full py-3.5 rounded-xl font-semibold text-sm" style={{ backgroundColor: "#f59e0b", color: "#0f1b2d" }} onClick={() => openReg("Complete Package")}>Register Now</button>
                <button className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl text-white font-semibold text-sm" style={{ backgroundColor: "#25D366" }} onClick={() => window.open(`https://wa.me/${WA}?text=${encodeURIComponent("Hi! I'm interested in the Complete Package for LUMS.")}`, "_blank")}><WAIcon />Chat on WhatsApp</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl overflow-hidden border border-gray-100 bg-white mt-6">
          <button onClick={() => setSchlOpen(!schlOpen)} className="w-full flex items-center gap-3 px-5 py-4 text-left">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: "#ecfdf5" }}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="7"/><polyline points="8.21 13.89 7 23 12 20 17 23 15.79 13.88"/></svg></div>
            <div className="flex-1"><h4 className="text-sm font-bold" style={{ color: "#0f1b2d" }}>LUMS Scholarships, Financial Aid, and On Campus Opportunities</h4></div>
            <ChevDown open={schlOpen}/>
          </button>
          {schlOpen && (<div className="px-5 pb-5">
            <div className="p-3 rounded-xl mb-4 flex gap-3 items-start" style={{ backgroundColor: "#fefce8" }}>
              <span className="text-lg flex-shrink-0">🚀</span>
              <div><h5 className="text-xs font-bold mb-0.5" style={{ color: "#92400e" }}>Extracurricular Edge</h5><p className="text-xs leading-relaxed" style={{ color: "#a16207" }}>Our EC developers build Math Olympiad preparation, Science competition portfolios, MUN leadership, and hackathon projects that directly strengthen your LUMS scholarship application</p></div>
            </div>
            <div className="flex flex-col gap-3">
              {scholarships.map((s, i) => (<div key={i} className="flex gap-3 items-start p-3 rounded-xl" style={{ backgroundColor: "#f8fafc" }}><div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 text-xs font-bold text-white" style={{ backgroundColor: "#059669" }}>{i + 1}</div><div><h5 className="text-sm font-semibold" style={{ color: "#0f1b2d" }}>{s.name}</h5><p className="text-xs mt-0.5 leading-relaxed" style={{ color: "#64748b" }}>{s.detail}</p></div></div>))}
            </div>
        ...
        <div className="rounded-2xl overflow-hidden border border-gray-100 bg-white mt-4">
          <button onClick={() => setProgOpen(!progOpen)} className="w-full flex items-center gap-3 px-5 py-4 text-left">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: "#e0f2fe" }}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#0ea5e9" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg></div>
            <div className="flex-1"><h4 className="text-sm font-bold" style={{ color: "#0f1b2d" }}>All LUMS Programs We Cover</h4></div>
            <ChevDown open={progOpen}/>
          </button>
          {progOpen && (<div className="px-5 pb-5 flex flex-col gap-3">
            {programs.map((p, i) => (<div key={i} className="p-3 rounded-xl" style={{ backgroundColor: "#f8fafc" }}>
              <h5 className="text-sm font-semibold" style={{ color: "#0f1b2d" }}>{p.school}</h5>
              <p className="text-xs mt-1" style={{ color: "#64748b" }}>{p.progs}</p>
              {p.school.includes("Education") && <span className="inline-block mt-2 text-xs font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: "#dbeafe", color: "#1d4ed8" }}>New: BS Educational Psychology</span>}
            </div>))}
          </div>)}
        </div>
      </section>
      ...
      {/* Trust */}
      <section className="px-4 py-10 max-w-5xl mx-auto">
        <h2 className="text-center text-xl font-bold mb-6" style={{ color: "#0f1b2d" }}>Why Families Trust SAT Sharks for LUMS</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { icon:"💰", title:"Milestone Based Payments", desc:"Fees divided into milestones. Satisfied with the previous one? Pay for the next. If not, you stop. Zero risk." },
            { icon:"🎯", title:"Honest Recommendation", desc:"We do not automatically push the Complete Package. If Guided Support is enough for your profile, that is what we recommend." },
            { icon:"🏆", title:"Strategy, Not Just Admission", desc:"We do not chase any offer. We build strategy around the LUMS program where your profile competes most effectively." }
          ].map((t, i) => (<div key={i} className="bg-white rounded-2xl border border-gray-100 p-5"><div className="text-2xl mb-3">{t.icon}</div><h3 className="text-sm font-bold mb-2" style={{ color: "#0f1b2d" }}>{t.title}</h3><p className="text-xs leading-relaxed" style={{ color: "#64748b" }}>{t.desc}</p></div>))}
        </div>
      </section>

      {/* CTA */}
      <section className="px-5 py-10 text-center" style={{ backgroundColor: "#0f1b2d" }}>
        <h3 className="text-white font-bold text-lg mb-2">98% of Our Students Got Into LUMS. You Could Be Next.</h3>
        <p className="text-xs leading-relaxed max-w-md mx-auto mb-6" style={{ color: "#94a3b8" }}>Everyone else polishes applications. We build applicants. From your Personal Statement to your competition trophies, we create the complete LUMS candidate. Don't take our word for it. Check our Instagram.</p>
      ...
      {/* Modal */}
      {showReg && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(15,27,45,0.7)", backdropFilter: "blur(4px)" }} onClick={e => { if (e.target === e.currentTarget) setShowReg(false); }}>
        <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl relative max-h-screen overflow-y-auto">
          <div className="px-6 pt-6 pb-4" style={{ background: "linear-gradient(135deg,#0f1b2d,#1a2744)" }}>
            ...
            <span className="inline-block text-xs font-bold tracking-widest uppercase px-3 py-1 rounded-full mb-3" style={{ backgroundColor: "#f59e0b", color: "#0f1b2d" }}>{selTier || "LUMS Admissions"}</span>
          ...
          {!done ? (<div className="px-6 py-6 flex flex-col gap-4">
            ...
            <div><label className="block text-xs font-semibold mb-1.5" style={{ color: "#0f1b2d" }}>WhatsApp Number</label><input value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} placeholder="03XX XXXXXXX" className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm outline-none focus:border-blue-400" /></div>
            <div><label className="block text-xs font-semibold mb-1.5" style={{ color: "#0f1b2d" }}>City *</label>
              <select value={form.city} onChange={e => setForm({...form, city: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm outline-none bg-white">{["","Lahore","Karachi","Islamabad","Rawalpindi","Peshawar","Faisalabad","Multan","Quetta","Other"].map(c => <option key={c} value={c}>{c || "Select city"}</option>)}</select>
            </div>
            <div><label className="block text-xs font-semibold mb-1.5" style={{ color: "#0f1b2d" }}>Target Program</label>
              <select value={form.program} onChange={e => setForm({...form, program: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm outline-none bg-white">{["","BBA","BSc Computer Science","BSc Electrical Engineering","BSc Mathematics","BSc Physics","BSc Biology","BSc Chemistry","BSc Economics and Mathematics","BA Economics","BA Political Science","BA Sociology","BA Psychology","BA English","BA History","BA Comparative Literary and Cultural Studies","BA LLB (Hons)","BS Educational Psychology","MBA","MS","MPhil Education","PhD","Not sure yet"].map(p => <option key={p} value={p}>{p || "Select program"}</option>)}</select>
            </div>
            ...
            <button onClick={() => { if (form.name && form.email && form.city) setDone(true); }} disabled={!form.name || !form.email || !form.city} className="w-full py-3.5 rounded-xl text-white font-semibold text-sm mt-1 disabled:opacity-40" style={{ backgroundColor: "#0f1b2d" }}>Submit Registration</button>
