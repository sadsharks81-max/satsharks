...
async function main() {
    ...
    const p = path_1.default.join(dir, "SAT Practice Test 12 Answer Key.pdf");
    const text = await extractLayoutText(p);
    const lines = text.split("\n");
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        if (line.includes("Q16") || line.includes("Q5") || line.includes("Q17")) {
            console.log(`Line ${i + 1}: "${line}"`);
            // Test the regex
            const isQShortWithSeparator = line.match(/^Q(\d+)(?:\s*[:|—\-.]|\s+([A-D])(?=[A-Z\s]|$))/i);
            console.log(`  regex match:`, isQShortWithSeparator ? isQShortWithSeparator[0] : "null");
        }
    }
}
