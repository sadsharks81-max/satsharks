...
function isIgnoredPracticeLine(line) {
    ...
    if (/^--\s*\d+\s*of\s*\d+\s*--$/i.test(trimmed))
        return true; // stand-alone page number tag
    if (/^DSAT Practice/i.test(trimmed))
        return true;
    if (/Original MCQs/i.test(trimmed))
        return true;
    if (/Solutions with Reasoning/i.test(trimmed))
        return true;
    if (/DIGITAL SAT READING/i.test(trimmed))
        return true;
    if (/Practice Questions/i.test(trimmed))
        return true;
    if (/DOC \d+/i.test(trimmed))
        return true;
    if (/QUESTIONS ONLY/i.test(trimmed))
        return true;
    if (/Detailed Solutions/i.test(trimmed))
        return true;
    if (/Evidence\s*,\s*/i.test(trimmed))
        return true;
    return false;
}
function parseOptionsFromLine(line, currentQ) {
...
function parseQuestions(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (mdMatch || textMatch) {
            ...
            currentQ = {
                questionNumber: qNum,
                difficulty: (diffStr === "EASY" || diffStr === "MEDIUM" || diffStr === "HARD" ? diffStr : "MEDIUM"),
                text: "",
                options: []
            };
...
function getSolutionsFile(qFile) {
...
function cleanCategoryName(qFile) {
    let base = qFile
        .replace(/\.pdf$/i, "")
        .replace(/\.docx$/i, "")
        .replace(/\.md$/i, "");
...
async function main() {
    ...
    for (const item of ORIGINAL_FILE_MAP) {
        itemsToImport.push({
            qPath: path_1.default.join(practicequestionsDir, item.questions),
            sPath: path_1.default.join(practicequestionsDir, item.solutions),
    ...
    // 2. NEW FOLDER 2 (Math)
    if (fs_1.default.existsSync(practicequestions2Dir)) {
        const files2 = fs_1.default.readdirSync(practicequestions2Dir).filter(f => !f.includes("(1)"));
        ...
        for (const qFile of questionFiles) {
            ...
            if (!sFile)
                continue;
            const qPath = path_1.default.join(practicequestions2Dir, qFile);
            const sPath = path_1.default.join(practicequestions2Dir, sFile);
            ...
            const ext = path_1.default.extname(qFile).substring(1);
            itemsToImport.push({
                qPath,
                sPath,
    ...
    for (const item of itemsToImport) {
        ...
        const categoryDoc = await QuestionCategory_1.default.findOneAndUpdate({ name: item.category }, {
            $set: {
                name: item.category,
        ...
        for (const q of parsedQuestions) {
            ...
            if (!sol) {
                console.warn(`  WARNING: No solution found for question #${q.questionNumber} in ${path_1.default.basename(item.qPath)}`);
                continue;
            }
