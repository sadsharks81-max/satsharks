...
const seedPhaseTwo = async () => {
    ...
    const allCats = await QuestionCategory_1.default.find();
    const catMap = new Map(allCats.map((c) => [c.name, c._id]));
    for (const q of sampleQuestions) {
        const categoryId = catMap.get(q.categoryName);
        if (!categoryId)
            continue;
        await Question_1.default.updateOne({ text: q.text }, {
            $set: {
                text: q.text,
                options: q.options,
    ...
    const mathQuestions = await Question_1.default.find({ section: "MATH", status: "PUBLISHED" }).limit(4);
    if (mathQuestions.length > 0) {
        await DiagnosticTest_1.default.updateOne({ title: "SAT Math Diagnostic – Quick Test" }, {
            $set: {
    ...
    const rwQuestions = await Question_1.default.find({ section: "READING_WRITING", status: "PUBLISHED" }).limit(2);
    if (rwQuestions.length > 0) {
        await DiagnosticTest_1.default.updateOne({ title: "SAT Reading & Writing – Mini Test" }, {
            $set: {
