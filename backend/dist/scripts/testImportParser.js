...
async function run() {
    ...
    for (const item of ORIGINAL_FILE_MAP) {
        const qPath = path_1.default.join(practiceDir, item.questions);
        const sPath = path_1.default.join(practiceDir, item.solutions);
        if (!fs_1.default.existsSync(qPath) || !fs_1.default.existsSync(sPath)) {
            console.error(`  ERROR: Files not found for ${item.category}`);
            totalErrors++;
            continue;
        }
        const qText = await extractText(qPath, item.type);
        const sText = await extractText(sPath, item.type);
    ...
    const files2 = fs_1.default.readdirSync(practiceDir2).filter(f => !f.includes("(1)"));
    ...
    for (const qFile of questionFiles) {
        ...
        if (!sFile || !fs_1.default.existsSync(path_1.default.join(practiceDir2, sFile))) {
            console.warn(`  WARNING: No solution file found for questions file "${qFile}" (expected "${sFile}")`);
            totalErrors++;
            continue;
        }
        const qPath = path_1.default.join(practiceDir2, qFile);
        const sPath = path_1.default.join(practiceDir2, sFile);
