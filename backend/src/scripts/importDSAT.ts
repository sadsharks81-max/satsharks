...
function shouldSkipLine(line: string): boolean {
  ...
  if (/^\d+$/i.test(clean)) return true;
  
  // 3. Section/Module Headers and instruction banners
  const lower = clean.toLowerCase();
  if (lower.startsWith("module 1") || lower.startsWith("module 2") || lower.startsWith("mod 1") || lower.startsWith("mod 2")) return true;
  if (lower.startsWith("section 1") || lower.startsWith("section 2")) return true;
  if (lower.startsWith("reading & writing") && lower.includes("module")) return true;
  if (lower.startsWith("math") && lower.includes("module")) return true;
  if (lower.includes("adaptive path") && lower.includes("questions")) return true;
  if (lower.includes("minutes") && lower.includes("questions") && lower.includes("|")) return true;
...
function parseQuestionsList(text: string): PdfParsedQuestion[] {
  const questions: PdfParsedQuestion[] = [];
  const lines = text.split("\n").map(l => l.trim());
  
  let currentQ: PdfParsedQuestion | null = null;
...
interface PdfParsedSolution {
  num: number;
  answer: string;
  explanation: string;
}

function parseSolutionsList(text: string): PdfParsedSolution[] {
  const solutions: PdfParsedSolution[] = [];
  const lines = text.split("\n").map(l => l.trim());
  
  let currentSol: PdfParsedSolution | null = null;
...
interface PdfParsedSkill {
  skill: string;
  difficulty: "EASY" | "MEDIUM" | "HARD";
}

function parseSkillsList(text: string): PdfParsedSkill[] {
  const list: PdfParsedSkill[] = [];
  const lines = text.split("\n");
  for (const line of lines) {
    ...
    if (cleanLine.toLowerCase().includes("skill:")) {
      ...
      if (match) {
        ...
        list.push({
          skill,
          difficulty: difficultyStr as "EASY" | "MEDIUM" | "HARD"
        });
      } else {
        list.push({
...
async function parsePdfTest(pdfPath: string, solutionPdfPath: string): Promise<ParsedModule[]> {
  ...
  if (parsedQs.length !== 147 || parsedSols.length !== 147 || parsedSkills.length !== 147) {
    throw new Error(`PDF parse length mismatch for ${path.basename(pdfPath)}. Qs: ${parsedQs.length}, Sols: ${parsedSols.length}, Skills: ${parsedSkills.length}`);
  }
...
async function main() {
  ...
  for (let tNum = 1; tNum <= 18; tNum++) {
    ...
    if (tNum <= 8) {
      ...
      let questionsPath = path.join(digitalsatpapersDir, `DSAT${fileNum}_text.txt`);
      if (!fs.existsSync(questionsPath)) {
        questionsPath = path.join(digitalsatpapersDir, `DSAT${fileNum}.md`);
      }

      let solutionsPath = path.join(digitalsatpapersDir, `DSAT${fileNum}sol_text.txt`);
      if (!fs.existsSync(solutionsPath)) {
        solutionsPath = path.join(digitalsatpapersDir, `DSAT${fileNum}sol.md`);
      }
      ...
      let fileAContent = fs.readFileSync(questionsPath, "utf-8");
      ...
      const pdfSource = path.resolve(digitalsatpapersDir, `DSAT${fileNum}.pdf`);
      ...
      const qPdfPath = path.join(digitalsatpapers2Dir, `SAT Practice Test ${tNum}.pdf`);
      const sPdfPath = path.join(digitalsatpapers2Dir, `SAT Practice Test ${tNum} Answer Key.pdf`);
