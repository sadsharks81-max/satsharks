...
function mapQuestionToModule(cfg: TestConfig, pageNum: number, qNum: number, section: string): string | null {
  ...
  if (cfg.testNumber === 12 && pageNum === 33) {
    if (section === "READING_WRITING") return "MOD2_HARD";
    return "MOD1";
  }
  
  if (section === "READING_WRITING") {
    if (pageNum >= q.rw_mod1[0] && pageNum <= q.rw_mod1[1]) return "MOD1";
    if (pageNum >= q.rw_mod2_easy[0] && pageNum <= q.rw_mod2_easy[1]) return "MOD2_EASY";
    if (pageNum >= q.rw_mod2_hard[0] && pageNum <= q.rw_mod2_hard[1]) return "MOD2_HARD";
  } else {
    if (pageNum >= q.math_mod1[0] && pageNum <= q.math_mod1[1]) return "MOD1";
    if (pageNum >= q.math_mod2_easy[0] && pageNum <= q.math_mod2_easy[1]) return "MOD2_EASY";
    if (pageNum >= q.math_mod2_hard[0] && pageNum <= q.math_mod2_hard[1]) return "MOD2_HARD";
  }
  return null;
}

function mapSolutionToModule(cfg: TestConfig, pageNum: number, qNum: number, section: string): string | null {
  // Upper bounds validation
  if (section === "READING_WRITING" && qNum > 27) return null;
  if (section === "MATH" && qNum > 22) return null;
  ...
  if (cfg.testNumber === 12 && pageNum === 16 && section === "MATH") {
    if (qNum === 22) return "MOD1";
    return "MOD2_EASY";
  }
  
  if (section === "READING_WRITING") {
    if (pageNum >= s.rw_mod1[0] && pageNum <= s.rw_mod1[1]) return "MOD1";
    if (pageNum >= s.rw_mod2_easy[0] && pageNum <= s.rw_mod2_easy[1]) return "MOD2_EASY";
    if (pageNum >= s.rw_mod2_hard[0] && pageNum <= s.rw_mod2_hard[1]) return "MOD2_HARD";
  } else {
    if (pageNum >= s.math_mod1[0] && pageNum <= s.math_mod1[1]) return "MOD1";
    if (pageNum >= s.math_mod2_easy[0] && pageNum <= s.math_mod2_easy[1]) return "MOD2_EASY";
    if (pageNum >= s.math_mod2_hard[0] && pageNum <= s.math_mod2_hard[1]) return "MOD2_HARD";
  }
  return null;
}

async function parsePdfTest(cfg: TestConfig, pdfPath: string, solutionPdfPath: string): Promise<ParsedModule[]> {
  console.log(`[Test ${cfg.testNumber}] Extracting question pages...`);
  const qPages = await extractPagesText(pdfPath);
  console.log(`[Test ${cfg.testNumber}] Parsing questions...`);
  const rawQs = parseQuestionsFromPages(qPages);
  
  console.log(`[Test ${cfg.testNumber}] Extracting solution pages...`);
  const sPages = await extractPagesText(solutionPdfPath);
  console.log(`[Test ${cfg.testNumber}] Parsing solutions...`);
  ...
  for (const q of rawQs) {
    const targetModule = mapQuestionToModule(cfg, q.pageNum, q.num, q.section);
    if (!targetModule) continue;
    
    const key = `${q.section}-${targetModule}-${q.num}`;
    if (seenQKeys.has(key)) {
      ...
      if (existingIdx !== -1) {
        ...
        if (q.text.length > existing.text.length) {
          mappedQs[existingIdx] = { ...q, moduleType: targetModule };
        }
      }
      continue;
    }
    seenQKeys.add(key);
    mappedQs.push({ ...q, moduleType: targetModule });
  }
  ...
  for (const s of rawSols) {
    const targetModule = mapSolutionToModule(cfg, s.pageNum, s.num, s.section);
    if (!targetModule) continue;
    
    const key = `${s.section}-${targetModule}-${s.num}`;
    if (seenSolKeys.has(key)) {
      ...
      if (existingIdx !== -1) {
        ...
        if (s.explanation.length > existing.explanation.length) {
          mappedSols[existingIdx] = { ...s, moduleType: targetModule };
        }
      }
      continue;
    }
    seenSolKeys.add(key);
    mappedSols.push({ ...s, moduleType: targetModule });
  }
  ...
  for (const mod of modules) {
    ...
    for (let qNum = 1; qNum <= totalQs; qNum++) {
      ...
      if (!q || !s) {
        throw new Error(`Missing question or solution for Test ${cfg.testNumber} ${mod.name} Q${qNum}`);
      }
...
async function main() {
  ...
  for (const cfg of testConfigs) {
    ...
    console.log(`\n=========================================`);
    console.log(`IMPORTING DSAT TEST #${tNum}`);
    console.log(`=========================================`);

    const qPdfPath = path.join(digitalsatpapers2Dir, `SAT Practice Test ${tNum} - Standardized.pdf`);
    const sPdfPath = path.join(digitalsatpapers2Dir, `SAT Practice Test ${tNum} - Standardized Answer Key.pdf`);

    if (!fs.existsSync(qPdfPath) || !fs.existsSync(sPdfPath)) {
      throw new Error(`PDF or Answer Key for Test ${tNum} not found.`);
    }

    // --- CLEAN PREVIOUS DATA FOR THIS TEST ONLY ---
    const delQs = await Question.deleteMany({ tags: `dsat-${tNum}` });
    console.log(`Deleted ${delQs.deletedCount} existing questions for Test ${tNum}.`);

    const delTests = await SATTest.deleteMany({ testNumber: tNum });
    console.log(`Deleted ${delTests.deletedCount} existing test documents for Test ${tNum}.`);

    const delDiag = await DiagnosticTest.deleteMany({ title: new RegExp(`^SAT Practice Test ${tNum}\\s*$`, 'i') });
    console.log(`Deleted ${delDiag.deletedCount} existing diagnostic test documents for Test ${tNum}.`);
    ...
    const parsedModules = await parsePdfTest(cfg, qPdfPath, sPdfPath);
    ...
    for (let idx = 0; idx < orderedModuleKeys.length; idx++) {
      ...
      if (!parsedMod) {
        throw new Error(`Could not find parsed module for Test ${tNum} ${keyConfig.name}`);
      }

      const questionIds: mongoose.Types.ObjectId[] = [];
      console.log(`Saving Test ${tNum} ${keyConfig.name} (${parsedMod.questions.length} questions)...`);
      ...
      for (const q of parsedMod.questions) {
        ...
        // SAFETY BREACH PROTECTION: Ensure we never touch tests 1 to 8
        const isProtected = [1,2,3,4,5,6,7,8].some(n => uniqueTag.startsWith(`dsat-${n}-`));
        if (isProtected) {
  ...
  console.log("Import of Tests 10, 11, and 12 completed successfully!");
