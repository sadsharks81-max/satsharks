...
async function main() {
  ...
  const p = path.join(dir, "SAT Practice Test 17 Answer Key.pdf");
  
  if (fs.existsSync(p)) {
    const text = await extractLayoutText(p);
    const solutions = parseSolutionsList(text);
    console.log(`Total parsed solutions: ${solutions.length}`);
    for (let idx = 0; idx < solutions.length; idx++) {
      const sol = solutions[idx];
      console.log(`[${idx}]: Q${sol.num} Answer="${sol.answer}" Expl="${sol.explanation.substring(0, 100)}"`);
    }
  }
}
